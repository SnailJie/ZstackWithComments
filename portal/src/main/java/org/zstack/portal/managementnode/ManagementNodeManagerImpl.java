...
/*
 * RenJie: 这里就是ManagementNode的入口代码，我们一起往下看。我们先看他的启动。直接看startNode（）
 */
public class ManagementNodeManagerImpl extends AbstractService implements ManagementNodeManager {
    ...
    public boolean start() {
        ...
        populateExtensions();   //RenJie: 这个还不知道干啥的
        ...
        try {
            ...
            FlowChain bootstrap = FlowChainBuilder.newSimpleFlowChain();      //RenJie: 开始执行神秘的任务链了。讲道理我还没搞清楚这个
            ...
            bootstrap.then(new Flow() {
                ...
                //RenJie: 有道理的， 如果节点在启动过程中出错，确实是需要进行回滚操作
                @Override
                public void run(FlowTrigger trigger, Map data) {
                    trigger.next();     //因为cloudBus已经在Platform.createComponentLoaderFromWebApplicationContext()初始化过了，所以这里直接过
                }                       //【云总线如何更新的？】
    ...
    public void startNode() {
        startInThread();     //把具体的启动过程放到其他线程里面运行。后面是死循环，等待MN（ManagementNode）启动完成。
